...
public function getFormId() {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $i=0;
    foreach ($entries = $this->load() as $entry) {
      $attributes = array('class' => array('link'), 'id' => array($entry['id']));
      $form['table'][$i][''] = array(
                                        '#type' => 'checkbox',
    ...
    $form['table'][$i]['Phone'] = array(
                                        '#type' => 'label',
                                        '#title' => t($entry['phone']),
                                        '#attributes' => $attributes,
                                    );
    $form['table'][$i]['Email'] = array(
                                        '#type' => 'label',
                                        '#title' => t($entry['email']),
                                        '#attributes' => $attributes,
                                    );
    $form['table'][$i]['#attributes'] = array(
                                        'class' => array('content'),
                                    );
    $i++;
    }
